    ...
    <div className="space-y-4 max-w-3xl mx-auto">
      {faqs.map((faq, index) => {
        ...
        return (
          <div
            key={index}
            className="border border-slate-100 bg-white rounded-2xl overflow-hidden shadow-premium transition-all duration-300"
          >
            <button
            ...
            <AnimatePresence initial={false}>
              {isOpen && (
                ...
                >
                  <div className="px-6 pb-6 pt-0 border-t border-slate-50 text-slate-600 leading-relaxed text-sm md:text-base">
                    {faq.answer}
                  </div>
                </motion.div>
              )}
